...
When using a hash table, collision handling is an important issue that needs to be addressed. Due to the fact that memory is scarce on the STM32H723 cuckoo hashing was not used due to it's increased memory consumption. After performing simulations on linear probing vs double hashing (simple and effective methods for this application) double hashing was implemented. However, there is one significant drawback with double hashing, and that is the effect load capacity still increases even with deletion of entry due to tombstoning. Tombstoning is required to allow access to entry which has collided and needed to be double hashed. This does reduce performance as the number of tombstones increase. To mitigate this the number of tombstones will be recorded as well as the number of entries in the data base. This will be used to monitor the effictive loading of the hash table. If the effective loading exceeds a predefined threshold the database will be reconstructed. However, this shouldn't happend too much since the database is already reconstructed everytime the phone is power cycled.
